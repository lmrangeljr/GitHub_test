...
FROM RAW_EXTRACTIONS_DEIDPROD.CRS_SYMPTOMS_D216_20240227.CRS_SYMPTOMS_COMBINED_RESULTS
WHERE CONTAINS(UPPER(EXTRACTION), 'FACE PAIN')
        OR CONTAINS(UPPER(EXTRACTION), 'FACIAL PAIN')
        OR CONTAINS(UPPER(EXTRACTION), 'FACE PRESSURE')
        OR CONTAINS(UPPER(EXTRACTION), 'FACIAL PRESSURE');

SELECT DISTINCT MENTION, EXTRACTION
FROM RAW_EXTRACTIONS_DEIDPROD.CRS_SYMPTOMS_D216_20240227.CRS_SYMPTOMS_COMBINED_RESULTS
WHERE CONTAINS(UPPER(EXTRACTION), 'NASAL');


SELECT DISTINCT CATEGORY, SYMPTOM_MAPPED, UPPER(MENTION)
FROM RAW_EXTRACTIONS_DEIDPROD.CRS_SYMPTOMS_D216_20240227.CRS_SYMPTOMS_COMBINED_RESULTS
ORDER BY UPPER(MENTION);
...
TT AS (SELECT DISTINCT MED.PATIENT_ID
			...
			,CASE WHEN ABS(DATEDIFF(DAY, PRC.PROC_DATE, MED.MEDICATION_START)) <= 30 THEN 1 ELSE 0 END AS WITHIN30DAYS
