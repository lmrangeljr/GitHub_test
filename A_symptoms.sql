...
/*******************************************************************************************************************
* SYMPTOMS
* MODIFCATIONS BY MUDIT THIS NO INCLUDE THE IS_NEGATED FLAG SET TO FALSE. 
* IT IS USING THE CATEGORY INSTEAD OF THE EXTRACTION NOTES.
*******************************************************************************************************************/
...
SELECT DISTINCT MAIN.PATIENT_ID
	...
	,CASE
		...
		ELSE 'NO DATA'
	END AS RUN_ID
	,COUNT (DISTINCT CASE WHEN UPPER(category) = 'NASAL_CONGESTION' AND is_negated = FALSE THEN BASE.NOTE_RECORDED_DATE ELSE NULL END) AS NASAL_OBSTR
	,COUNT (DISTINCT CASE WHEN UPPER(category) = 'HYPOSMIA_ANOSMIA' AND is_negated = FALSE THEN BASE.NOTE_RECORDED_DATE ELSE NULL END) AS ANOSMIA
	,COUNT (DISTINCT CASE WHEN UPPER(category) = 'FACIAL_PAIN' AND is_negated = FALSE THEN BASE.NOTE_RECORDED_DATE ELSE NULL END) AS FACIAL_PAIN
	,COUNT (DISTINCT CASE WHEN UPPER(category) = 'NASAL_DRAINAGE' AND is_negated = FALSE THEN BASE.NOTE_RECORDED_DATE ELSE NULL END) AS NASAL_DISCHR
...
select *
from RESEARCH.GSK_CRSWNP.A_SYMPTOMS_LMRJR
limit 100;